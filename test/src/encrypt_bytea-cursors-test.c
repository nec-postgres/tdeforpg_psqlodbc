...
//	rc = SQLExecDirect(hstmt, (SQLCHAR *) "SELECT 'foo'::encrypt_bytea || g FROM generate_series(1, 3210) g", SQL_NTS);
	rc = SQLExecDirect(hstmt, (SQLCHAR *) "SELECT 'foo'::encrypt_bytea FROM generate_series(1, 3210) g", SQL_NTS);
