...
int main(int argc, char **argv)
{
	...
	rc = SQLExecDirect(hstmt,
					   (SQLCHAR *) "CREATE TABLE enc_btest (len int4, b encrypt_bytea)",
					   SQL_NTS);
	...
	/* Prepare a statement */
	rc = SQLPrepare(hstmt, (SQLCHAR *) "INSERT INTO enc_btest VALUES(?, ?)", SQL_NTS);
	CHECK_STMT_RESULT(rc, "SQLPrepare failed", hstmt);
	...
	rc = SQLExecDirect(hstmt,
					   (SQLCHAR *) "SELECT len, length(b) FROM enc_btest",
					   SQL_NTS);
	CHECK_STMT_RESULT(rc, "SQLExecDirect failed", hstmt);
	print_result(hstmt);
