 ...
 * Modified to test TDEforPG's encrypt_numeric. 
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
test_numeric_result(HSTMT hstmt, char *numstr)
{
	...
	snprintf(sql, sizeof(sql), "SELECT '%s'::encrypt_numeric", numstr);
...
int main(int argc, char **argv)
{
	...
	/* pgtde_begin_session */
	rc = SQLExecDirect(hstmt, (SQLCHAR *) "SELECT pgtde_begin_session('aofiafoaeiaofijabeoptuiotaoi')", SQL_NTS);
	CHECK_STMT_RESULT(rc, "SQLExecDirect failed while begin_session", hstmt);
	...
	rc = SQLPrepare(hstmt, (SQLCHAR *) "SELECT ?::encrypt_numeric", SQL_NTS);
